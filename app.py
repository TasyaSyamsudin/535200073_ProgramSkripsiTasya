...
def add_data():
    try:
        if request.method == 'POST':
            ...
            print("Nilai new_id:", new_id)
            new_data = request.form['new_data']

            # Menambahkan data ke MySQL
            connection = mysql.connector.connect(**mysql_config)
            cursor = connection.cursor()
            query = "INSERT INTO dataset (id, data) VALUES (%s, %s)"
            cursor.execute(query, (new_id, new_data,))
            connection.commit()
            connection.close()

            return redirect(url_for('view_data'))
    except ValueError as ve:
        print(f"Error: {ve}. Input value: {request.form['new_id']}")
        # Atau berikan pesan kesalahan kepada pengguna, misalnya:
        # flash("Error: ID or data is not a valid integer.")
    except Exception as e:
        print("Error:", str(e))
        # Atau berikan pesan kesalahan umum.
...
def edit_data(data_id):
    ...
    if request.method == 'POST':
        ...
        edited_id = request.form['edited_id']
        edited_data = request.form['edited_data']

        # Memperbarui data di MySQL
        connection = mysql.connector.connect(**mysql_config)
        cursor = connection.cursor()
        query = "UPDATE dataset SET id = %s, data = %s WHERE id = %s"
        cursor.execute(query, (edited_id, edited_data, data_id))
        connection.commit()
        connection.close()
...
def delete_data(data_id):
    # Menghapus data dari MySQL berdasarkan ID
    connection = mysql.connector.connect(**mysql_config)
    cursor = connection.cursor()
